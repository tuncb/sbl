...
func renderDocumentBody(section, slug, sourcePath, markdown string, localAssetURLs map[string]string) (template.HTML, []assets.File, int, error) {
	markdownInput, mermaidBlocks, err := ExtractMermaid(markdown)
	if err != nil {
		return "", nil, 0, fmt.Errorf("%s: %w", sourcePath, err)
	}
	...
	if err := markdownEngine.Convert([]byte(markdownInput), &buffer); err != nil {
		return "", nil, 0, fmt.Errorf("%s: render markdown: %w", sourcePath, err)
	}
	...
	if err != nil {
		return "", nil, 0, fmt.Errorf("%s: rewrite asset links: %w", sourcePath, err)
	}
